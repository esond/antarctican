...
- **SWAG** (in `media/`) is the reverse proxy / TLS terminator for `antarctican.tv` (wildcard cert obtained via DNS-01 through Cloudflare; Cloudflare handles DNS and cert validation only — traffic resolves straight to the WAN IP and never routes through Cloudflare's proxy). It owns ports 80/443/81 on the host. Other stacks expose their UIs on non-standard ports and SWAG fronts them — keep that in mind when changing ports. The only tracked nginx conf is `media/swag/proxy-confs/dashboard.subdomain.conf`, copied to `${APPDATA}/swag/nginx/proxy-confs/` on the host (see `media/README.md`) — it deliberately has **no 443 server block** (the mod's stock conf has one, which made the dashboard public); keep it that way.
