...
- **SWAG** (in `media/`) is the reverse proxy / TLS terminator for the domain in `${SWAG_URL}` (wildcard cert obtained via DNS-01 through Cloudflare; Cloudflare handles DNS and cert validation only — traffic resolves straight to the WAN IP and never routes through Cloudflare's proxy). It owns ports 80/443/81 on the host. Other stacks expose their UIs on non-standard ports and SWAG fronts them — keep that in mind when changing ports. The only tracked nginx conf is `media/swag/proxy-confs/dashboard.subdomain.conf`, copied to `${APPDATA}/swag/nginx/proxy-confs/` on the host (see `media/README.md`) — it deliberately has **no 443 server block** (the mod's stock conf has one, which made the dashboard public); keep it that way.
...
- **otel** (in `otel/`) is the telemetry sink for every other stack. The collector is the only intake: OpenClaw pushes OTLP to its published port, and everything else is scraped through `host.docker.internal` on its **published host port**, never by joining another stack's network — that keeps stacks independently startable. Some targets expose Prometheus `/metrics` natively (unpackerr, flaresolverr, cloudflared, couchdb); the rest get an exporter sidecar that lives in the stack owning the service, with its credentials in that stack's `.env` (qbittorrent-exporter and scraparr in `media/`, pihole-exporter in `pihole/`, urbackup-exporter in `urbackup/`). The `plex/` stack is deliberately not instrumented. When you add a metrics endpoint in another stack, publish it with a `${SERVICE_METRICS_HOST_PORT}` var there, add the matching port var to `otel/.env.example`, **pass that var into the collector's `environment:`** in `otel/docker-compose.otel.yml` (miss this and the job silently resolves to an empty target), and add a scrape job in `otel/otel-collector/config.yaml`. The collector, Tempo, Loki and Grafana configs under `otel/` are tracked here and copied into `${APPDATA}` on the host (see `otel/README.md`), same pattern as the SWAG dashboard conf.
